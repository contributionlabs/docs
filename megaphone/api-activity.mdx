...
- `<queryParamKey?><queryParamValue>`: identity-specific static query params. Supported queryParams are `evmAddress` and `emailAddress`. If you would like other identity options, please request them on our [public Megaphone roadmap](https://megaphone.canny.io/).
  ...
  <Danger>
    ...
    \
    For `emailAddress`, note that Megaphone automatically blocks signups from [temporary/disposable email services](https://raw.githubusercontent.com/disposable/disposable-email-domains/master/domains.txt) as part of our anti-sybil protections.
  </Danger>
